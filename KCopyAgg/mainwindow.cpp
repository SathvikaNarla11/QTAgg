    ...
    ,iconSize(38, 38)
{
    ...
    //    connect(ui->graphicsView, &CustomGraphicsView::mouseDoubleClick, this, &MainWindow::onGraphicsViewMouseDoubleClicked);
...
void MainWindow::onGraphicsViewMousePressed(QMouseEvent *event)
{
    ...
    else if (event->button() == Qt::RightButton)
    {
        ...
        if (shapeType == CustomShapeItem::Rectangle || shapeType == CustomShapeItem::Ellipse || shapeType == CustomShapeItem::Line)
        {
            ...
            QAction *lineWidthAction = contextMenu->addAction(QIcon(":/Icons/RightClickShape/lineWidth.png"), "Line Width");
            ...
            connect(lineWidthAction, &QAction::triggered, this, &MainWindow::changeLineWidth);
...
void MainWindow::changeLineWidth()
{
    ...
    {
        bool ok;
        int newWidth = QInputDialog::getInt(this, tr("Line Width"),tr("Enter new line width:"),1, 1, 100, 1, &ok);
        if (ok)
        {
            QPen pen;
            if (QGraphicsLineItem* lineItem = dynamic_cast<QGraphicsLineItem*>(clickedItem))
            {
                qDebug() << "LineItem.";
                pen = lineItem->pen();
            ...
            else if (CustomShapeItem* customItem = dynamic_cast<CustomShapeItem*>(clickedItem))
            {
                qDebug() << "CustomItem.";
                pen = customItem->pen();
                pen.setWidth(newWidth);
            ...
            {
                qDebug() << "Clicked item is not a recognized shape.";
            }
        }
    }
}
...
//        {
//            CustomShapeItem* lineItem = new CustomShapeItem(CustomShapeItem::Line);

//            lineItem->setShapeLine(QLineF(origin, origin));
//            scene->addItem(lineItem);
//            currentItem = lineItem;

//            // Add handles at the endpoints of the line
////            addEndpointsHandles(lineItem);
//        }
//        else
//        {
//            currentItem = new CustomShapeItem(shapeType);
...
//            if (shapeType == CustomShapeItem::Line)
//            {
//                // Update endpoints of the line
//                QLineF line = resizableShapeItem->getShapeLine();
//                if (handle == endpoints[0])
//                {
//                    line.setP1(currentPos);
//                }
//                else if (handle == endpoints[1])
//                {
//                    line.setP2(currentPos);
//                }
//                resizableShapeItem->setShapeLine(line);
//            }
//            else
//            {
//                QRectF originalRect = resizableShapeItem->boundingRect();
...
//        if (shapeType == CustomShapeItem::Line)
//        {
//            addEndpointsHandles(resizableShapeItem);
//        }
//        else
//        {
//            addBlueHandles(resizableShapeItem);
...
//void MainWindow::addEndpointsHandles(CustomShapeItem* lineItem)
//{
//    QLineF line = lineItem->getShapeLine();
//    endpoints.clear();
//    endpoints.append(addHandleAt(line.p1(), Qt::blue, lineItem));
//    endpoints.append(addHandleAt(line.p2(), Qt::blue, lineItem));
//}
