    ...
    ,iconSize(38, 38)
{
    ...
    {
        QWidget* firstTab = ui->tabWidget->widget(0); // Assuming tab0 is the first tab
        QTabWidget* innerTabWidget = new QTabWidget(firstTab);

        // Create a new QWidget for the first inner tab and use ui->graphicsView
        QWidget* innerTab1 = new QWidget();
        QVBoxLayout* innerTabLayout = new QVBoxLayout(innerTab1);
        innerTabLayout->setContentsMargins(0, 0, 0, 0);
        innerTabLayout->addWidget(ui->graphicsView); // Add ui->graphicsView to innerTab1
        innerTab1->setLayout(innerTabLayout);

        // Add the inner tab to innerTabWidget
        ...
        connect(innerTabWidget, &QTabWidget::currentChanged, [this, innerTabWidget](int index) {
            if (innerTabWidget->tabText(index) == "Add New") {
                ...
                CustomGraphicsView* graphicsView = new CustomGraphicsView(newInnerTab);
                QGraphicsScene* scene = new QGraphicsScene(graphicsView);
                graphicsView->setScene(scene);
                scene->setSceneRect(graphicsView->rect());
                graphicsView->setRenderHints(QPainter::Antialiasing);
                QVBoxLayout* layout = new QVBoxLayout(newInnerTab);
                layout->setContentsMargins(0, 0, 0, 0);
                layout->addWidget(graphicsView);
                newInnerTab->setLayout(layout);

                innerTabWidget->insertTab(innerTabWidget->count() - 1, newInnerTab, QString("Inner Tab %1").arg(innerTabWidget->count()));
                innerTabWidget->setCurrentIndex(innerTabWidget->indexOf(newInnerTab));
            }
        });

        // Set layout to the firstTab widget and add the innerTabWidget
        if (firstTab->layout() == nullptr) {
...
void MainWindow::onGraphicsViewMouseReleased(QMouseEvent *event)
{
    if (drawing && event->button() == Qt::LeftButton)
    {
        ...
        if (shapeType == CustomShapeItem::Rectangle || shapeType == CustomShapeItem::Ellipse || shapeType == CustomShapeItem::Line)
        {
            ...
            if (shapeItem)
            {
                if (shapeType == CustomShapeItem::Line)
                {
                    QLineF line(origin, ui->graphicsView->mapToScene(event->pos()));
                    shapeItem->setShapeLine(line);
                }
                else
                {
...
//        ui->tabWidget->setTabText(index, QString("page #%1").arg(tabCount));
...
void MainWindow::on_tabWidget_tabBarClicked(int index)
{
    ...
    if (index == tabCount - 1)
    {
        ...
        scene1->setSceneRect(graphicsView1->rect());
        graphicsView1->setRenderHints(QPainter::Antialiasing);
        QVBoxLayout* layout1 = new QVBoxLayout(innerTab1);
        ...
        // Set layout to the newTab widget and add the innerTabWidget
        QVBoxLayout* layout = new QVBoxLayout(newTab);
        layout->setContentsMargins(0, 0, 0, 0);
        layout->addWidget(innerTabWidget);
        newTab->setLayout(layout);

        // Add the newTab widget as a new tab
...
{
    zoomFactor /= 1.5;
    ui->graphicsView->scale(1.0 / 1.5, 1.0 / 1.5);
}


void MainWindow::on_actionZoom_to_Normal_triggered()
{
